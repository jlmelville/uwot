...
`mnist`, `fashion`, `kuzushiji`, `norb`, `cifar10`, `hierarchical`,
`isofaces`, `isoswiss`, `mammoth`, `scurvehole`, `spheres` and (an
unprocessed version of) `ng20` can be downloaded or generated via the
[snedata](https://github.com/jlmelville/snedata) package. `coil20` and
`coil100` can be downloaded via the
[coil20](https://github.com/jlmelville/coil20) package. Other data I
processed using Python. Notebooks for these can be found at
<https://github.com/jlmelville/drnb/tree/master/notebooks/data-pipeline>.
...
unable to find code for generating it in the PaCMAP repo. The [gitub
repo](https://github.com/hyhuang00/scRNA-DR2020/) for the paper
[“Towards a comprehensive evaluation of dimension reduction methods for
transcriptomic data
visualization”](https://doi.org/10.1038/s42003-022-03628-x) (by many of
the same authors), contains a numpy-formatted version of the data under
the name `hierarchical_threelayer_dataset`, and we can probably assume
it’s the data also used in the PaCMAP paper. The paper describes the
generation like so:

> consists of 5 “macro clusters” that each have 5 “meso clusters,” and
> each of the meso subclusters has 5 “micro clusters.” Therefore, there
> are 5x5x5 = 125 micro clusters total. We colored each point based on
> its true macro clusters, shading each meso cluster.”.

Get all of that? There’s more detail in Appendix B of the PacMAP paper.
