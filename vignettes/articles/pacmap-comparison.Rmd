...
example). If speed matters to you than running `tumap` or `umap2` with 
`a = 1, b = 1` and `negative_sample_rate = 4.0` will close the gap a bit. On the 
other hand, if speed really matters, then PaCMAP will give you vital seconds
back without having to fiddle with the UMAP defaults.
