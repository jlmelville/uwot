...
mnist_lv <- lvish(mnist, perplexity = 50, init = "lvrand", n_epochs = 1000, 
                  verbose = TRUE)
...
`n_epochs`, is set to `1000`, is still much larger than for `umap`, but leads to
a lot less sampling than the official LargeVis implementation, but may need to
be increased further depending on your dataset. That value seems to be ok with
MNIST. Using the UMAP initialization of `init = "spectral"`, rather than the
default Gaussian random initialization (`init = "lvrand"`) can help.
